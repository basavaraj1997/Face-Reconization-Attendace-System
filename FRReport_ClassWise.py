...
class FRReport_ClassWise:
    def __init__(self,root):
        ...
        title_lb=Label(bg_img,text="Class Wise Report",font=("times new roman",20,"bold"),bg="white",fg="black")
...
#==========================First Table================================================
    ...
    def GetBindDataToTable(self):
        ...
        while 0 <= num_months:
            ...
            try:
                #self.stud_table.delete(*self.stud_table.get_children())
                query2=" select rollno,fullname "+A_PCases+"  from (select to_Char(a.odt,'dd') dd,s.srno,s.fullname,s.rollno from student_mst s left join attendace a on a.srno=s.srno and s.rollno=a.rollno"
                query2=query2+" and a.odt between to_char(to_date('"+FirsDate+"','dd-mm-yyyy'),'dd-MON-yyyy') and to_char(to_date('"+EndDate+"','dd-mm-yyyy'),'dd-MON-yyyy'))"
    ...
    def export(self):
        if(self.ToDt.get()=="" or self.frmDt.get()==""):
            messagebox.showinfo('Information', "Select Date!!!")
            return
        ...
        d_1, m_1, y_1 = [int(x) for x in self.frmDt.get().split('-')]
        d_2, m_2, y_2 = [int(x) for x in self.ToDt.get().split('-')]
        
        fpath=open_file
        fullpath=fpath+'\Attendace'+str(m_1)+'_'+str(m_2)+'.xlsx' #Get Both Date
